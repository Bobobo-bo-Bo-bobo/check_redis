...
void usage(void) {
    std::cout << "check_redis_master version " << CHECK_REDIS_VERSION << std::endl;
    std::cout << std::endl;
    std::cout << "Copyright (c) by Andreas Maus <maus@ypbind.de>" << std::endl;
    std::cout << "This program comes with ABSOLUTELY NO WARRANTY." << std::endl;
    std::cout << std::endl;
    std::cout << "check_redis_master is distributed under the terms of the GNU General" << std::endl;
    std::cout << "Public License Version 3. (http://www.gnu.org/copyleft/gpl.html)" << std::endl;
    std::cout << std::endl;
    std::cout << "Check if Redis instance runs as master" << std::endl;
    std::cout << std::endl;
    std::cout << "Usage: check_redis_master [-h|--help] -H <host>|--host=<host> [-p <port>|--port=<port>]" <<std::endl;
...
int main(int argc, char **argv) {
    ...
    std::cout << "Server runs as master" << std::endl;
    return STATUS_OK;
}
